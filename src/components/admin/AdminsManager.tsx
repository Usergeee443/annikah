    ...
    <div className="grid gap-4">
      {canManage ? (
        <section className="rounded-3xl border border-zinc-200/70 bg-white p-5 ring-1 ring-zinc-200/70">
          <div className="text-[11px] font-semibold tracking-wide text-zinc-500">YANGI HISOB</div>
          <h2 className="mt-1 text-[14px] font-bold tracking-normal text-zinc-950">Katta admin yoki moderator</h2>
          ...
          <div className="mt-4 grid gap-3 lg:grid-cols-[1fr_1fr_minmax(140px,180px)_160px_140px]">
            <label className="grid">
              <span className="text-[10px] font-semibold uppercase tracking-wide text-zinc-500">Username</span>
              <input
            ...
            <label className="grid">
              <span className="text-[10px] font-semibold uppercase tracking-wide text-zinc-500">Parol</span>
              <input
                className={inputCls + " mt-1"}
                value={password}
            ...
            <label className="grid">
              <span className="text-[10px] font-semibold uppercase tracking-wide text-zinc-500">Rol</span>
              <select
                className={inputCls + " mt-1"}
                value={role}
                onChange={(e) => {
            ...
            <div className="flex items-end">
              <button
                ...
                className="inline-flex h-11 w-full items-center justify-center rounded-2xl bg-zinc-950 px-4 text-[12px] font-semibold text-white ring-1 ring-black/10 transition hover:bg-zinc-900 disabled:opacity-60"
              >
      ...
      <section className="rounded-3xl border border-zinc-200/70 bg-white p-5 ring-1 ring-zinc-200/70">
        <div className="text-[11px] font-semibold tracking-wide text-zinc-500">RO‘YXAT</div>
        <h2 className="mt-1 text-[14px] font-bold tracking-normal text-zinc-950">Adminlar ({admins.length})</h2>
              ...
              >
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="text-[14px] font-bold tracking-normal text-zinc-950">{a.username}</span>
                    <span
                      className={
                    ...
                    {a.role !== "super" && a.role !== "super_admin" && a.gender ? (
                      <span className="inline-flex items-center rounded-full bg-sky-50 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-sky-900 ring-1 ring-sky-200">
                        {a.gender === "female" ? "ayol moderator" : "erkak moderator"}
                      </span>
                    ) : null}
                    {a.id === currentSessionId ? (
                      <span className="inline-flex items-center rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-emerald-800 ring-1 ring-emerald-200">
                        SIZ
                      </span>
                    ) : null}
                  </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {canManage && a.id !== currentSessionId ? (
                    <button
                      ...
                      disabled={pending}
                      className="inline-flex h-9 items-center justify-center rounded-2xl bg-rose-600 px-3 text-[12px] font-semibold text-white ring-1 ring-rose-700/20 hover:bg-rose-700 disabled:opacity-60"
                    >
                      O‘chirish
                    </button>
                  ) : null}
