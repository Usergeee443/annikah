...
export default function ChatRoom({
  ...
  function onPickedFile(file: File | null) {
    ...
    startUpload(async () => {
      try {
        ...
        fd.append("file", file);
        const res = await fetch(`/api/chats/${chatId}/messages`, { method: "POST", body: fd });
        const data = await res.json().catch(() => null);
        if (!res.ok) {
          const code = data?.error;
          if (code === "UNSUPPORTED_MEDIA_TYPE") throw new Error("Rasm turi noto‘g‘ri (JPEG/PNG/WebP).");
          if (code === "FILE_TOO_LARGE") throw new Error("Rasm juda katta.");
  ...
  return (
    <section className="flex h-full min-h-0 flex-1 flex-col overflow-hidden bg-white lg:rounded-3xl lg:border lg:border-zinc-200/70 lg:shadow-[0_8px_28px_rgba(15,23,42,.06)]">
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {m.kind === "image" ? (
                          <img
                            src={`/api/chats/${chatId}/messages/${m.id}/media`}
                            alt=""
      ...
      <div className="lg:static lg:shrink-0 lg:border-t lg:border-zinc-200/70 lg:bg-white/95 lg:backdrop-blur">
        <div
          className="fixed inset-x-0 bottom-0 z-20 bg-transparent px-3 py-3 sm:px-5 lg:static lg:px-0 lg:py-3"
        ...
        {isEnded ? (
          ...
          <div className="mx-auto flex w-full max-w-3xl items-end gap-2 rounded-3xl bg-white/92 p-2 shadow-[0_14px_35px_rgba(15,23,42,.18)] ring-1 ring-zinc-200/80 backdrop-blur">
            <input
              ref={fileRef}
              type="file"
            ...
            />
            <div className="flex flex-1 items-end rounded-2xl bg-zinc-100 px-2 py-1 transition focus-within:bg-white focus-within:shadow-[0_0_0_3px_rgba(24,24,27,.06)] focus-within:ring-1 focus-within:ring-zinc-200">
              <button
                type="button"
                onClick={pickImage}
                disabled={uploading || pending}
                aria-label="Rasm yuborish"
                className="mr-1 inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-zinc-500 transition hover:bg-zinc-200/80 hover:text-zinc-700 disabled:opacity-60"
                title="Rasm"
              >
                <svg viewBox="0 0 24 24" className="h-4.5 w-4.5" fill="none">
                  ...
                  />
                  <path
                    d="M8 14l2-2 2 2 4-4 2 2v3H6v-3z"
                    stroke="currentColor"
                    strokeWidth="1.8"
                    strokeLinejoin="round"
                  />
              ...
              <textarea
                ...
                className="max-h-40 min-h-[24px] w-full resize-none bg-transparent py-2 text-[14px] font-medium text-zinc-900 outline-none placeholder:text-zinc-400"
              />
            </div>
            <button
              type="button"
