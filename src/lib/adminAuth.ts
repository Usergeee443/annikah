...
export async function assertCanModerateListing(listingId: number | string) {
  const s = await requireAdmin();
  const id = typeof listingId === "number" ? listingId : parseListingIdParam(String(listingId));
  if (id === null) throw new Error("NOT_FOUND");
