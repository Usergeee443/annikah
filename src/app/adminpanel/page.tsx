...
function Stat({
  ...
  return (
    <div className="rounded-3xl bg-white p-5 ring-1 ring-zinc-200/70 shadow-[0_8px_28px_rgba(15,23,42,.05)]">
      ...
      <div className="mt-2 text-xl font-bold tracking-normal text-zinc-950">{value}</div>
  ...
  return (
    <div className="grid gap-4">
      <div className="rounded-3xl border border-zinc-200/70 bg-white p-5 shadow-[0_8px_28px_rgba(15,23,42,.05)]">
        <div className="text-[11px] font-semibold tracking-wide text-zinc-500">ADMINPANEL</div>
        <h1 className="mt-2 text-xl font-bold tracking-normal text-zinc-950">Statistika</h1>
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        <div className="rounded-3xl border border-zinc-200/70 bg-white p-5 ring-1 ring-zinc-200/70">
          <div className="flex items-center justify-between gap-2">
            <div className="text-[11px] font-semibold tracking-wide text-zinc-500">TARIFLAR</div>
            <Link
              href="/adminpanel/pricing"
              className="text-[11.5px] font-semibold text-zinc-700 hover:text-zinc-950"
            >
              O‘zgartirish →
            </Link>
          </div>
            ...
            {pricing.listingPlans.map((p) => (
              ...
              >
                <div className="min-w-0">
                  <div className="text-[13.5px] font-semibold text-zinc-950">{p.title}</div>
                  <div className="text-[11.5px] font-semibold text-zinc-500">{p.days} kun</div>
                </div>
                <div className="text-[14px] font-bold text-zinc-950">
                  {p.priceUzs.toLocaleString()} <span className="text-[11px] font-semibold text-zinc-500">so‘m</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="rounded-3xl border border-zinc-200/70 bg-white p-5 ring-1 ring-zinc-200/70">
          ...
          <div className="mt-3 grid gap-2">
            {pricing.boosts.map((b) => (
              ...
              >
                <div className="min-w-0">
                  <div className="text-[13.5px] font-semibold text-zinc-950">{b.label}</div>
                  <div className="text-[11.5px] font-semibold text-zinc-500">{b.days} kun</div>
                </div>
                <div className="text-[14px] font-bold text-zinc-950">
                  {b.priceUzs.toLocaleString()} <span className="text-[11px] font-semibold text-zinc-500">so‘m</span>
                </div>
              </div>
            ))}
          </div>
