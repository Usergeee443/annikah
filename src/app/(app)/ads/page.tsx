...
export default async function AdsPage() {
  ...
  const [viewsAgg, likesAgg] = sortedIds.length
    ...
    : [[], []];
  const viewsById = new Map<number, number>(viewsAgg.map((x) => [x.listingId, x._count._all]));
  const likesById = new Map<number, number>(likesAgg.map((x) => [x.listingId, x._count._all]));
    ...
    <div className="grid gap-6">
      ...
      <div className="rounded-3xl bg-linear-to-r from-zinc-50/90 via-white to-zinc-50/80 px-1 py-5 sm:px-2">
        <div className="grid gap-4 sm:flex sm:flex-wrap sm:items-center sm:gap-7">
          <div className="flex items-center gap-5 sm:flex-1 sm:gap-7">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-2 max-w-xl text-[13px] font-medium leading-relaxed text-zinc-600 sm:text-[14px]">
                Reklamadagi e’lonlarda <span className="font-semibold text-zinc-900">“Top”</span> belgisi ko‘rinadi va
          ...
          <Link
            href="/reklama"
            className="inline-flex h-10 w-full items-center justify-center gap-2 rounded-2xl bg-linear-to-r from-amber-500 via-orange-500 to-rose-500 px-6 text-[12px] font-semibold text-white ring-1 ring-orange-600/30 shadow-[0_14px_34px_rgba(249,115,22,.22)] transition hover:from-amber-400 hover:via-orange-500 hover:to-rose-500 sm:h-10 sm:w-auto sm:px-7"
          >
      ...
      {listings.length === 0 ? (
        <div className="flex min-h-[calc(100dvh-10rem)] flex-col items-center justify-center text-center">
          ...
          <div className="mt-4 text-[16px] font-bold tracking-normal text-zinc-950">Reklamingiz yo‘q</div>
          <p className="mt-1 max-w-sm text-[13px] font-medium text-zinc-600">
            Boost uchun avval e’lon bo‘lishi kerak.
          </p>
          <Link
            href="/reklama"
            className="mt-6 inline-flex h-10 items-center justify-center rounded-2xl bg-zinc-950 px-6 text-[12px] font-semibold text-white ring-1 ring-black/10 hover:bg-zinc-900"
          >
            Reklama berish
          </Link>
        </div>
